...
<div class="card">
    <div class="card-body">
        ...
        <ul class="nav nav-tabs nav-bordered mb-3">
            ...
            <li class="nav-item">
                <a href="/dataGuruHonorer" class="nav-link active">
                   Guru Honorer
                </a>
            </li>
            <li class="nav-item">
        ...
        <h2>Data Guru Honorer</h2>
        <div class="col-lg-2">
            <button type="button" class="btn btn-info" data-bs-toggle="modal" data-bs-target="#tambah">Tambah Gaji Guru </button>
        </div>   
        <hr>    
        <table id="datatable-buttons" class="table table-striped dt-responsive nowrap w-100 mt-2 px-2">
            ...
            <tbody>
                @foreach($guru as $g)
                <?php 
                    $diff = date_diff( $g->pegawai->created_at, $tanggal );
                ?>
                <tr class="text-center">
                    ...
                    <td>
                        <a href="/editGuruHonorer/{{ Crypt::encrypt($g->id) }}" class="btn-sm btn-primary">Edit</a>
                        <form action="/hapusGuruHonorer/{{  Crypt::encrypt($g->id) }}" method="post" class="d-inline">
                            @method('delete')
                            @csrf
                ...
                <tr class="text-center">
                    ...
                    <td>Belum Ada</td>
                    <td>
                        {{-- <a href="/editGuruHonorer/{{ Crypt::encrypt($p->id) }}" class="btn-sm btn-primary">Edit</a>
                        <form action="/hapusGuruHonorer/{{  Crypt::encrypt($p->id) }}" method="post" class="d-inline">
                            @method('delete')
                            @csrf
                            <button class="btn-sm btn-danger border-0"  onclick="return confirm('Are you sure?')">Hapus
                              </button>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="tambahLabel">Tambah Gaji Guru Honorer</h4>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-hidden="true"></button>
            </div>
            <form action="/tambahGajiHonorer" method="post">
