...
<div class="card">
    <div class="card-body">
        ...
        <ul class="nav nav-tabs nav-bordered mb-3">
            <li class="nav-item">
                <a href="/gaji" class="nav-link active">
                   Bulan ini
                </a>
            </li>
            <li class="nav-item">
                <a href="/rekapGajiGuru" class="nav-link">
                    Rekap Gaji
                </a>
            </li>
        </ul> <!-- end nav-->
            ...
            <tbody>
                @foreach ($gaji as $g)
                    <tr>
                        ...
                        <td>Rp. {{number_format($g->mapel->gaji ) }}</td>
                        <td>{{ $g->tanggal_penggajian }}</td>
                        <td>{{ $g->status }}</td>
                        <td>
                            <a href="/detailGajiGuru/{{  Crypt::encrypt($g->id) }}" class="btn-sm btn-info">Detail </a>
                            <form action="/hapusGajiGuru/{{  Crypt::encrypt($g->id) }}" method="post" class="d-inline">
                                @method('delete')
                                @csrf
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <form action="/inputGajiPegawai/{{  Crypt::encrypt($p->id) }}" method="post" class="d-inline">
                            @csrf
                            <button class="btn-sm btn-success border-0"  onclick="return confirm('Are you sure?')">Input
                              </button>
                        </form>
                    </td>
                </tr>
...
<div id="buat-absen" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="buat-absenLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="/tambahGajiGuru" method="post">
