...
<div class="card">
    <div class="card-body">
        ...
        <ul class="nav nav-tabs nav-bordered mb-3">
            ...
            <li class="nav-item">
                <a href="/dataGuruPPPK" class="nav-link active">
                   Guru PPPK
                </a>
            </li>
            <li class="nav-item">
        ...
        <h2>Data Guru PPPK</h2>
        <div class="col-lg-2">
            <button type="button" class="btn btn-info" data-bs-toggle="modal" data-bs-target="#tambah">Tambah Guru PPPK</button>
        </div>   
        <hr>    
        <table id="datatable-buttons" class="table table-striped dt-responsive nowrap w-100 mt-2 px-2">
            ...
            <tbody>
                ...
                <tr class="text-center">
                    ...
                    <td>{{ $g->skhpppk->pppk->golongan_pppk }}</td>
                    <td>Rp. {{ number_format($g->skhpppk->gaji_pppk ) }}</td>
                    <td>
                        <a href="/editGuruPPPK/{{ Crypt::encrypt($g->id) }}" class="btn-sm btn-primary">Edit</a>
                        <form action="/hapusGuruPPPK/{{  Crypt::encrypt($g->id) }}" method="post" class="d-inline">
                            @method('delete')
                            @csrf
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="tambahLabel">Tambah Guru PPPK</h4>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-hidden="true"></button>
            </div>
            <form action="/tambahGuruPPPK" method="post">
                                    ...
                                    <div class="input-group">
                                        <select class="form-select" name="skh_pppk_id" id="example-select">
                                            <option selected disabled>Pilih Golongan PPPK</option>
                                                @foreach ($pppk as $p)
                                                    <option value="{{ $p->id }}">{{ $p->pppk->golongan_pppk }}</option>
                                                @endforeach
                                        </select>
                                    </div>
                                </div>
